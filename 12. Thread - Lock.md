...
- Solution : ```setpark()```
  ...
  - ```park()```로 잠드는 것이 아닌, 즉시 ```lock()``` 함수를 끝내버리도록 되어있다.
...
![image](https://user-images.githubusercontent.com/71700079/168472846-7d49e595-4d9e-4c6a-a220-5523b12f4fba.png)  
![image](https://user-images.githubusercontent.com/71700079/168472852-2430e4e2-807a-4e76-b54e-06e6cd2fcc39.png)  
- ```int v```의 최상위 1bit만을 사용해서, Lock 상태를 제어한다.
  - 누군가가 Lock을 사용중이라면, ```mutex```가 음수(최상위 bit 1)가 된다.
  - 마지막 ```v >= 0```으로 한번 더 확인했을 때, v가 음수이면 누군가가 사용중이기 때문에 ```futex_wait()```로 재운다.
  - v가 양수이면 아무도 쓰지 않는다는 것! ```continue``` 시키고 ```while(1)```문의 제일 위에서 Lock을 얻는다.
- ```futex_wait(address, expected)```
  - Thread를 재우는 기능을 한다.
  - Address와 Expected가 다르면, 재우지 않고 그냥 즉시 return 해버린다.
- ```futex_wake(address)```
  - Unlock시에 잠들었던 Thread를 깨운다.
- ```unlock()``` 코드에 있는 ```0x80000000```을 더하는 동작은 무엇일까?
  - 현재 Lock이 사용중이고, 대기열에 아무도 없다면(0x800000000) 깨우지 않고 Return 하면 된다.
  - 즉, ```futex_wake(mutex)``` 위에서 그런 이유로 Return 시켜버리는 것이다.
